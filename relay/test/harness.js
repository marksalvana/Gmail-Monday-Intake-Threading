 ...
 * Test harness.
 *
 * Loads Relay.gs — THE EXACT FILE THAT GETS PASTED — into a VM sandbox and
...
function loadSandbox() {
  const file = path.join(__dirname, '..', 'Relay.gs');
  const sandbox = { console, JSON, Math, Date, String, Number, Object, Array,
                    RegExp, Error, isNaN, parseInt, parseFloat, Buffer };
  sandbox.global = sandbox;
  vm.createContext(sandbox);
  new vm.Script(fs.readFileSync(file, 'utf8'), { filename: 'Relay.gs' }).runInContext(sandbox);
  return sandbox;
}
